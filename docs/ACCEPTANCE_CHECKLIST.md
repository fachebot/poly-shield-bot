...
- 左侧存在 `仓位` / `存档仓位` 两个 tab。
- 当某 token 不在实时持仓但存在历史任务时，出现在 `存档仓位`。
- 存档项显示只读标记。

通过标准：

- tab 可切换且数据正常渲染。
- 存档项可打开查看历史任务，但不允许新建/编辑。

### 11.2 自动回迁

步骤：

1. 让一个 token 先处于 `存档仓位`。
2. 在该 token 重新建仓（可通过外部手动买回）。
3. 等待自动刷新周期或手动刷新仓位列表。

通过标准：

- 该 token 自动从 `存档仓位` 回到 `仓位`。
...
- 左侧仓位列表定时自动刷新时应为静默更新。
- 用户主动切换 tab 时应显示加载反馈并暂时禁用操作。

通过标准：

- 没有用户操作时不会突然弹出加载动画遮罩。
- 手动切换时有加载动画，加载完成后恢复交互。
