...
7. Web 控制台可正确展示持仓/存档仓位、任务详情和时间线
...
1. 左侧有 `仓位` / `存档仓位` tab，并可切换。
2. 选择 token 后右侧任务板可刷新并展示对应任务。
3. 打开任务详情后，规则时间线与系统事件可展开查看。
4. 时间线为分页懒加载，滚动到底可自动加载更多。

通过标准：

- 页面可正常加载，tab 切换和任务选择无明显报错。
- 存档 token 显示只读，不允许新建/编辑任务。
- 规则记录和 system 记录都能在详情里看到。

### JT-07 Web 控制台交互体验

验证点：

1. 用户主动切换左侧或右侧 tab 时，出现加载动画并临时禁用交互。
2. 左侧仓位列表定时自动刷新为静默更新，不出现突兀动画。
